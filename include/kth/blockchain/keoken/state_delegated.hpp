...
struct state_delegated {
    ...
    using create_asset_func = std::function<void(std::string /*asset_name*/, amount_t /*asset_amount*/, kd::wallet::payment_address const& /*owner*/, size_t /*block_height*/, kth::hash_digest const& /*txid*/)>;
    using create_balance_entry_func = std::function<void(asset_id_t /*asset_id*/, amount_t /*asset_amount*/, kd::wallet::payment_address const& /*source*/, kd::wallet::payment_address const& /*target*/, size_t /*block_height*/, kth::hash_digest const& /*txid*/)>;
    using asset_id_exists_func = std::function<bool(asset_id_t /*id*/)>;
    using get_balance_func = std::function<amount_t(asset_id_t id, kd::wallet::payment_address const& addr)>;
    using get_assets_by_address_func = std::function<get_assets_by_address_list(kd::wallet::payment_address const& /*addr*/)>;
